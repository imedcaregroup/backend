...
export class RemainingSlotsService {
    ...
    public setEntityParameters(medicalId: number|null, employeeId: number|null): void {
    ...
    private getRemainingDays(bookedSlotsByDate: any, availableSlots: any): Array<any> {
        const remainingDays = [];
        const now = new Date();
        const currentDay: number = now.getDate();

        for (let day = currentDay; day <= this.getLastDayOfMonth(); day++) {
            const date = new Date(this.getChosenYear(), this.getChosenMonth() - 1, day, 0, 0, 0);
            ...
            (slot: any) =>
                ...
                bookedSlotsOnDate?.[String(slot.startTime)] >=
                    ...
                    (availableSlot: any) => availableSlot.startTime === slot.startTime,
                    ).length),
            );
        ...
        return await __db.order.findMany({
            where: {
                ...
                orderDate: {
                    ...
                    lte: new Date(this.getChosenYear(), this.getChosenMonth() - 1, this.getLastDayOfMonth()),
                },
    ...
    private ensurePassedMedicalOrEmployee(): void {
        if (!this.entityParameters.medicalId && !this.entityParameters.employeeId) {
            throw new Error("You should pass one of the following cases: medicalId or employeeId with medicalId");
        }
    }
    ...
    private isToday(day: number): boolean {
        ...
        return this.getChosenYear() === currentYear && this.getChosenMonth() === currentMonth && day === currentDay;
    }

    private getLastDayOfMonth(): number {
        return new Date(this.dateParameters.year, this.dateParameters.month - 1, 0).getDate()
    }
