...
export const createAdminValidation = [
  ...
  check("role").optional().isIn(['ADMIN', 'SUPER_ADMIN']).withMessage("Role must be either ADMIN or SUPER_ADMIN"),
];
...
export const changePasswordValidation = [
  check("currentPassword").notEmpty().withMessage("Please provide current password"),
  check("newPassword").notEmpty().withMessage("Please provide new password")
    .isLength({ min: 6 }).withMessage("Password must be at least 6 characters long"),
  check("confirmPassword").notEmpty().withMessage("Please confirm new password")
    .custom((value, { req }) => {
